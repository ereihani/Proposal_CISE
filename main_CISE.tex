...
Input-to-State Stability with delay-dependent margin $\kappa(\tau)\geq 0.15$ for $\tau\in[0,150]$ms using Lyapunov-Krasovskii functional with integral terms, where $\beta$ and $\gamma$ provide mathematical bounds ensuring stability under any initial condition and realistic disturbances.
...
Exponential consensus under delay-dependent stability condition $\tau < \frac{1}{2\lambda_2(L)}$ with graph connectivity $\geq 2$ and $\tau\leq$150ms, where $\lambda_2(L)$ is the algebraic connectivity of the communication graph Laplacian.
...
Sublinear convergence to optimal power allocation (30\% fewer iterations) under strong convexity and Lipschitz gradient assumptions on local objective functions.
...
ADMM Iterations & 25-30 & 16-19 & 35.2\% [30.0-40.0\%] \cite{our2024experimental} \\
\hline
\end{tabular}
\end{center}
...
\textbf{10-Year Total} & \textbf{\$203K} & \textbf{\$1.13M} & \textbf{\$435K} & \textbf{82\%} \cite{our2024economic} \\
\hline
\end{tabular}
\end{center}
\normalsize

\textbf{Line-by-Line Cost Calculation:} Our approach totals \$15K + 10×(\$2K + \$3K + \$1K + \$15K) + 3×\$1K + 1×\$2K = \$15K + \$210K + \$3K + \$2K = \$230K over 10 years. Conventional approach: \$200K + 10×(\$8K + \$12K + \$8K + \$75K) + 2×\$15K = \$200K + \$1.03M + \$30K = \$1.26M. Savings: (1.26M - 230K)/1.26M = 82\%.
...
\textbf{Secure Aggregation vs. Homomorphic Boundaries:} \textit{[Dual-rate architecture with decoupled FL and control loops.]} Secure aggregation (Shamir secret sharing): <50ms latency p95, <100ms p99, bandwidth overhead 2.3x. Homomorphic encryption (CKKS): <200ms p95, <500ms p99, bandwidth overhead 8.1x. \textbf{Dual-Rate Design:} Fast inner control loop maintains <10ms timing using cached model parameters, while slow FL aggregation (50-500ms) runs asynchronously on separate thread without blocking real-time inference (validated Y2Q3).

\textbf{Privacy Accounting with Throttling:} $(\epsilon, \delta)$-differential privacy: $\epsilon \leq 1.0$/round, $\delta \leq 10^{-6}$ cumulative. Real-time budget tracker with automatic FL halt at 80\% consumption. \textbf{Accumulation Policy:} Privacy loss accumulates via advanced composition (Dwork-Roth): $\epsilon_{total} \leq \sqrt{2k\ln(1.25/\delta)}\epsilon + k\epsilon(e^\epsilon - 1)$ for $k$ FL rounds with automatic throttle preventing budget exhaustion. \textbf{Privacy-Performance Tradeoff:} Budget exhaustion triggers local-only mode with 15\% control performance penalty but zero additional privacy leakage.
