...
\textbf{CBF Infeasibility Handling:} When QP constraints become infeasible under extreme disturbances, our framework employs slack variable relaxation: $u_{safe} = \arg\min_u ||u - u_{nom}||^2 + \gamma||slack||^2$ subject to $\nabla h(x) \cdot (f(x) + g(x)u + f_{ML}(x; \theta)) + \alpha h(x) \geq -slack$, with penalty weight $\gamma = 10^4$. Preliminary HIL validation demonstrates infeasibility rate $<$1\% under realistic fault scenarios, ensuring robust constraint satisfaction.
