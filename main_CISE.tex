...
\textbf{OPERATIONAL CONTRACTS \textcolor{blue}{[Envelope A--C]}:} \textbf{QP solve time:} $<$10ms (99\% confidence), violations $<$2/hour (95\% confidence), system availability $>$99.5\% (measured monthly). \textbf{Contract breach auto-response:} When violated (e.g., QP $>$10ms p99), system automatically (i) disables ML components, (ii) activates certified LMI controller, (iii) widens barriers; HMI logs event and notifies ops within 1 min. \textbf{Fallback performance guarantee:} $<$10\% degradation during safety mode transitions.
\end{minipage}}
\end{center}
