...
Input-to-State Stability with delay-dependent margin $\kappa(\tau) = \kappa_0 - c\tau \geq 0.15$ for $\tau\in[0,150]$ms using Lyapunov-Krasovskii functional $V(x_t) = V_0(x(t)) + \int_{-\tau}^0 x^T(t+s)Qx(t+s)ds$ with integral terms. The derivation follows: (1) Model delayed dynamics $\dot{x}(t) = f(x(t), x(t-\tau)) + g(x(t))u + w(t)$; (2) Choose nominal Lyapunov $V_0(x) = x^T P x$ (P>0 from LMI), augmented with Krasovskii integral for delays; (3) Ensure $\dot{V} \leq -\kappa V + \gamma ||w||^2$ via physics-informed constraints, where $\beta$ and $\gamma$ provide mathematical bounds ensuring stability under any initial condition and realistic disturbances.
...
Exponential consensus under delay-dependent stability condition $\tau < \frac{1}{2\sqrt{\lambda_2(L)}}$ with graph connectivity $\lambda_2(L) \geq 0.01$, step-size $\alpha < \frac{1}{\lambda_{max}(L)}$, and $\tau\leq$150ms. The derivation: (1) Dynamics $\dot{\eta_i} = -\alpha \sum_j a_{ij}(\eta_i(t-\tau) - \eta_j(t-\tau)) + \text{RL term}$; (2) Error $e = \eta - \mathbf{1}\eta_{avg}$ yields $\dot{e} = -\alpha L e(t-\tau)$; (3) Lyapunov $V = e^T e$ with $\dot{V} \leq -2\alpha\lambda_2 V + O(\tau^2||\dot{e}||^2)$; (4) Stability requires $\alpha\tau < \frac{1}{2\sqrt{\lambda_2}}$ from small-gain theorem, where $\lambda_2(L)$ is the algebraic connectivity of the communication graph Laplacian.
...
Sublinear convergence to optimal power allocation (empirically 30\% fewer iterations) under convex local objectives. Linear convergence $\mathcal{O}(\rho^K)$ achievable under Assumptions A-D: strong convexity of local functions, Lipschitz gradients, proper penalty $\rho > \lambda_{max}(A^TA)$, and graph connectivity $\lambda_2(L) > 0$. The derivation: (1) Problem $\min \sum_i f_i(z_i)$ s.t. $Az = b$ (power balance); (2) Augmented Lagrangian $L_\rho = \sum f_i + y^T(Az-b) + \frac{\rho}{2}||Az-b||^2$; (3) ADMM updates with residual $r^k = Az^k - b$; (4) Under strong $\mu$-convexity and Lipschitz $L$-gradients: $||r^k|| \leq (1-\min(\mu/\rho, \rho/L))^k O(1)$ for properly tuned $\rho$. GNN acceleration via learned graph embeddings.
...
Finds control closest to AI preference while mathematically guaranteeing safety limits are never exceeded, with heavy penalty $\gamma\geq$10$^4$ for constraint violations. The CBF method: (1) Define safe set $h(x) \geq 0$ (e.g., $h = 0.5^2 - \Delta f^2$ for frequency bounds); (2) Ensure forward invariance via $\dot{h} = \frac{\partial h}{\partial x}\dot{x} + \alpha h \geq 0$ with class-$\mathcal{K}$ function $\alpha > 0$; (3) If $u_{nom}$ violates safety, solve QP to find minimum deviation $u_{safe}$ that satisfies barrier constraints. This integrates with ISS framework treating safety violations as bounded disturbances.
