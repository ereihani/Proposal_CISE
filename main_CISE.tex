...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figure6_safety_verification.pdf}
\caption{Control Barrier Function Safety Verification}
\label{fig:safety_verification}
\end{figure}

Figure \ref{fig:safety_verification} demonstrates the comprehensive safety verification capabilities of our Control Barrier Function framework across four critical dimensions. Panel (a) shows the evolution of multiple barrier functions over time, with frequency, voltage, and angle barriers maintaining positive values throughout the simulation, ensuring continuous operation within safe bounds. The green shaded region indicates the safe operating zone, with all barrier functions remaining above the red safety boundary (h(x) = 0), validating the forward invariance property that guarantees perpetual safety once initialized in a safe state.

Panel (b) visualizes the two-dimensional safe operating region in the frequency-voltage phase space, where the elliptical safety boundary defines admissible operating conditions. The blue trajectory demonstrates how the CBF framework maintains system evolution within safe bounds even under severe disturbances, with the system starting from the green point and converging to the red endpoint while never violating safety constraints. This visualization confirms that the barrier function $h(x) = 0.25 - (\Delta f)^2 - ((V-1.0)/0.1)^2$ successfully defines a practical safe operating region for real-world deployment.

Panel (c) illustrates the control input modification mechanism, comparing nominal control commands (red dashed line) with CBF-filtered safe control inputs (blue solid line). During periods when the nominal controller would violate safety constraints, the CBF optimization framework automatically modifies control signals to maintain operation within the green safe region. This demonstrates the transparent safety enforcement capability, where the CBF layer intervenes only when necessary while minimizing performance degradation during normal operation.

Panel (d) presents the critical N-2 contingency response validation, showing system frequency evolution during a severe fault where two critical components fail simultaneously at t=5s. Despite the significant disturbance, the CBF framework successfully maintains frequency deviations within the ±0.5 Hz safety limits (red dashed lines), demonstrating robust performance under extreme conditions. The orange shaded recovery phase shows controlled system restoration, validating the framework's ability to handle the most challenging scenarios while preventing cascading failures that could compromise grid stability.
